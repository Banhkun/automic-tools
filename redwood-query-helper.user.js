...
// @version      0.11.0
// @description  Adds tabbed, reusable SQL query templates to the Redwood /redwood/support/query page (e.g. "find parent job chain for a list of job definitions"). v0.11.0: queries now submit via fetch/AJAX instead of a full page reload.
...
(function () {
    ...
    /* -----------------------------------------------------------------------
     * -1. IN-MEMORY TAB STATE
     * -------------------------------------------------------------------
     * Replaces the old sessionStorage-only "last submitted tab" tracking.
     * Since Generate & Run / Submit Query no longer trigger a full page
     * reload, the state that drives the reorder/graph/matrix enhancers can
     * live in a plain JS variable instead of round-tripping through
     * sessionStorage on every query. sessionStorage is still written on each
     * successful query (see submitQueryViaFetch) purely as a fallback for a
     * genuine manual page reload (F5).
     * ---------------------------------------------------------------------*/
    let activeTabId = null;      // updated by selectTab(); which tab's panel is currently visible
    let pendingTabState = null;  // set by a Generate & Run click just before submit; consumed once by the form's submit handler
    let currentTabState = null;  // authoritative "what produced the results currently on screen"
    ...
    function parseListKeepDuplicates(raw) {
        ...
        return raw
            .split(/[\n,]/)
            .map((s) => s.trim()); // intentionally keeps '' entries - blank pasted lines become spacer rows
    }
     ...
     * via getSavedTabState(), backed by the in-memory currentTabState / the
     * sessionStorage fallback), so it never misfires on other tabs' result
     * tables.
     * ---------------------------------------------------------------------*/
  ...
  `;
    ...
    function addWorkflowGraphView(table) {
        ...
        if (!state || state.id !== 'list-steps') return;
    ...
    // Returns whatever tab/query produced the results currently on screen.
    // Prefers the in-memory currentTabState (set right after a successful
    // AJAX query submission); falls back to sessionStorage only for a
    // genuine manual page reload (F5), where the in-memory state was lost.
    function getSavedTabState() {
        if (currentTabState) return currentTabState;
        try {
    ...
    const LIST_PARAMETERS_NAMES = [
        'ABAP_PROGRAM_NAME', 'ABAP_VARIANT_NAME', 'ARCHIVE_ARCTEXT', 'ARCHIVE_AR_OBJECT',
        'ARCHIVE_INFO', 'ARCHIVE_SAP_OBJECT', 'CLIENT', 'DELETE_JOB', 'DRAFT_MODE',
        'EMAIL_ADDRESS', 'IGNORE_APPL_RC', 'JOBCLASS', 'JOBCOUNT', 'JOBNAME', 'LANGUAGE',
        'NO_FRAME', 'NO_SHADE', 'PAGE_ORIENTATION', 'PRINT_ARMOD', 'PRINT_FOOTL',
        'PRINT_LICT', 'PRINT_LISZ', 'PRINT_PAART', 'PRINT_PDEST', 'PRINT_PEXPI',
        'PRINT_PLIST', 'PRINT_PRABT', 'PRINT_PRBER', 'PRINT_PRBIG', 'PRINT_PRCOP',
        'PRINT_PRDSN', 'PRINT_PRIMM', 'PRINT_PRNEW', 'PRINT_PRREC', 'PRINT_PRREL',
        'PRINT_PRSAP', 'PRINT_PRTXT', 'PRINT_PRUNX', 'PRINT_PTYPE', 'REC_BLIND_COPY',
        'REC_COPY', 'REC_DELIVER', 'REC_EXPRESS', 'REC_MAILSTATUS', 'REC_NO_FORWARDING',
        'REC_NO_PRINT', 'REC_RECIPIENT', 'REC_TYPE', 'SAP_SYSTEMS', 'SAP_USER_NAME',
        'SHOWAPPLLOG', 'SHOWLOG', 'SHOWSPOOL', 'SMS', 'TARGET_GROUP', 'TARGET_SERVER',
        'TEMPORARY_VARIANT',
    ];
    const LIST_PARAMETERS_DEFAULT_CHECKED = new Set(['ABAP_PROGRAM_NAME', 'ABAP_VARIANT_NAME']);
    ...
    function pivotParameterRows(rows) {
        ...
        rows.forEach(({ jd, param, value }) => {
            if (!jdSeen.has(jd)) { jdSeen.add(jd); jdOrder.push(jd); }
            if (!paramSeen.has(param)) { paramSeen.add(param); paramOrder.push(param); }
    ...
    function addMatrixHeaderCopy(table, toastAnchor) {
        ...
        Array.from(thead.rows[0].cells).forEach((th, idx) => {
            ...
            th.addEventListener('click', () => {
                const values = Array.from(tbody.rows).map((tr) => tr.cells[idx]?.textContent ?? '');
                navigator.clipboard.writeText(values.join('\n')).then(() => {
                    showRQHToast(toastAnchor, `Copied ${values.length} value(s) from "${th.textContent.trim()}"`);
                });
            });
        });
    }
    ...
    function downloadMatrixAsCsv(pivot) {
        const { jdOrder, paramOrder, cellMap } = pivot;
        const lines = [['JobDefinitionName', ...paramOrder].map(csvEscape).join(',')];
        jdOrder.forEach((jd) => {
            const row = [jd, ...paramOrder.map((p) => cellMap.get(jd + '\u0000' + p) ?? '')];
    ...
    function addParameterMatrixView(table) {
        ...
        btn.addEventListener('click', () => {
            ...
            btn.textContent = showingMatrix ? '⊞ Show as matrix' : '🔼 Hide matrix (show raw rows)';
        });
    ...
    function scanAndAddParameterMatrix() {
        document.querySelectorAll('table.report-outside').forEach(addParameterMatrixView);
    }
...
WHERE jd.Name IN (${inList})
  ...
  AND jd.BranchedLLPVersion = -1`;
    ...
    /* -----------------------------------------------------------------------
     * 1h. AJAX QUERY SUBMISSION (no full page reload)
     * -------------------------------------------------------------------
     * Historically every "Generate & Run" / "Submit Query" click was a real
     * form POST, causing a full page navigation. That wiped all script state
     * (hence the sessionStorage round-tripping used everywhere above) and
     * lost scroll position / collapsed states on every single query.
     *
     * Now the form's `submit` event is intercepted once (see init()) and
     * replaced with a fetch() POST. The response HTML is parsed off-screen
     * via DOMParser, and just the "results region" - everything that lives
     * after the <form> (Query Runtime / Query Row Count blocks + the result
     * table) - is swapped into a stable `#rqh-results-content` container.
     * The <form> itself (and the tab UI wrapper living inside it) is never
     * touched or replaced.
     *
     * Falls back to a real form.submit() (i.e. the old reload behavior) if
     * the fetch/parse fails for any reason, so a network hiccup never leaves
     * the user stuck with a dead button.
     * ---------------------------------------------------------------------*/

    // Wraps everything that comes after the query <form> - the Query
    // Runtime/Row Count blocks and the result table - in one stable
    // container, so future query submissions can swap just that region
    // instead of reloading the whole page. The static <style> block with
    // the report table's CSS is deliberately left in place (not moved into
    // the container) since it's identical on every response and only needs
    // to exist once.
    function wrapResultsRegion(form) {
        const existing = document.getElementById('rqh-results-content');
        if (existing) return existing;

        const container = document.createElement('div');
        container.id = 'rqh-results-content';
        ...
        while (node) {
            ...
            if (node.nodeType === Node.ELEMENT_NODE && node.tagName === 'STYLE') {
                // Leave the static report stylesheet where it is - don't move it.
            } else {
    ...
    // Runs every enhancer pass. Called directly after an AJAX results swap
    // as a belt-and-braces measure in case the MutationObserver (still
    // watching document.body) doesn't reliably fire for this kind of
    // childList replacement - all the scan* functions are idempotent per
    // table (they gate on dataset flags), so calling them twice is harmless.
    ...
    // Submits the form via fetch instead of a full page POST/reload, then
    // swaps the parsed response's results region into the live page.
    async function submitQueryViaFetch(form, tabState) {
        const resultsContainer = wrapResultsRegion(form);
        ...
        try {
            ...
            const newForm = doc.getElementById('queryEdit')?.closest('form');
            if (!newForm) throw new Error('Unexpected response - #queryEdit not found');
            ...
            newNodes.forEach((n) => resultsContainer.appendChild(document.importNode(n, true)));

            currentTabState = tabState;
            try {
                sessionStorage.setItem('rqh-active-tab', JSON.stringify(tabState));
            } catch (e) {
                /* non-fatal */
            }
            ...
            console.error('[RQH] AJAX query submit failed, falling back to full page reload:', err);
            form.submit();
        }
    }
    ...
    function init() {
        ...
        // Intercept every submission of this form (both the tabs' "Generate &
        // Run" buttons and the native "Submit Query" button on the Free
        // Query tab route through this same <form>) and send it via fetch
        // instead of letting the browser navigate.
        form.addEventListener('submit', function (e) {
            e.preventDefault();
            const tabState = pendingTabState || { id: activeTabId };
            pendingTabState = null;
            submitQueryViaFetch(form, tabState);
        });
        ...
        TAB_CONFIGS.forEach((cfg) => {
            ...
            if (cfg.inputLabel) {
                ...
                runBtn.addEventListener('click', () => {
                    ...
                    const secondaryValue = panel._secondaryInput ? panel._secondaryInput.value : '';
                    const secondaryList = panel._secondaryInput ? parseList(secondaryValue) : null;
                    const sql = cfg.buildQuery(list, input.value, direction, checked, secondaryList);
                    if (!sql) {
                    ...
                    textarea.value = sql;
                    // Stash the pending tab state for the form's submit handler to
                    // pick up - form.submit() bypasses the 'submit' event, so we
                    // use requestSubmit() (falling back to submit() on very old
                    // browsers, which will just do a full reload instead).
                    pendingTabState = { id: cfg.id, inputValue: input.value, direction, checked, secondaryValue };
        ...
        // Restore whichever tab was active before the last full page reload
        // (only relevant on a genuine F5 / fresh navigation - normal query
        // submissions no longer reload the page, so this mainly matters on
        // first load after a manual refresh).
        let restored = null;
        try {
            const saved = sessionStorage.getItem('rqh-active-tab');
        ...
        if (restored && panels[restored.id]) {
            ...
            if (panel._secondaryInput && typeof restored.secondaryValue === 'string') {
        ...
        // Wrap whatever's already in the results region (blank on a fresh
        // page, or a prior query's results if the page loaded with one) so
        // the container exists from the start and future AJAX swaps have
        // somewhere stable to land.
        wrapResultsRegion(form);
    }
